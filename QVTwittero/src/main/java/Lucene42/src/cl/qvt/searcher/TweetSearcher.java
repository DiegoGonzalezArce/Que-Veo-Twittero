 ...
 */
package Lucene42.src.cl.qvt.searcher;
...
public class TweetSearcher {
     ...
     * @throws ParseException 
     */
    public List<Integer> searchDocIds(IndexReader reader,List<String> Term,String label,int maxHits) throws IOException, ParseException{
        List<ScoreDoc> tweets=search(reader,Term,label,maxHits);
        List<Integer> result=new ArrayList<Integer>();
        for (Iterator iter = tweets.iterator(); iter.hasNext();){
            ScoreDoc elem = (ScoreDoc) iter.next();
     ...
     * @throws ParseException 
     */
    public List<String> searchTweetIds(IndexReader reader,List<String> Term,String label,int maxHits) throws IOException, ParseException{
        List<Integer> docs=searchDocIds(reader,Term,label,maxHits);
        IndexSearcher searcher = new IndexSearcher(reader);
        List<String> result=new ArrayList<String>();
        int i=0;
